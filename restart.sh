#!/usr/bin/env bash
# Stop any running MikuPrompt and start a fresh one in the background.
# Uses a pidfile so it never has to guess from process names.
set -uo pipefail
cd "$(dirname "$0")"

PIDFILE=".miku.pid"
LOG="${MIKU_LOG:-/tmp/mikuprompt.log}"
...
if curl -fsS --max-time 4 http://127.0.0.1:3939/api/health > /dev/null 2>&1; then
  echo "MikuPrompt up on http://127.0.0.1:3939  (pid $(cat "$PIDFILE"), log $LOG)"
else
